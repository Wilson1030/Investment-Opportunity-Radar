...
`INGEST_DRY_RUN=true` 时，区分点是「**事实 vs 判断**」而不是「是否写库」：

| 写入？ | 内容 | 理由 |
|:---:|---|---|
| ✅ | `IngestRun`（带 `dry_run=true`） | 否则 dry-run 期间看不到漏斗与质量指标 |
| ✅ | Company / Stock / Announcement / Paragraph | **事实**，幂等且无害 |
| ❌ | Event / Evidence / Thesis / Opportunity / Score / OpenQuestion / Alert | **判断**，未经校准不应进入系统 |

> **为什么改成这样**：早期实现连公告也不写，导致后续 LLM 抽取阶段没有输入
> （`_pending_announcements` 为空），实测 `events_extracted = 0`、`llm.calls = 0`。
> 结果是 dry-run **恰恰看不到 R3（模型能否胜任）最需要的数据**。
> 采集、解析、LLM 调用全部真实执行，报告 dump 到 `data/cache/dry_run/{date}/*.json`。

连续观察 **3 个交易日**，检查 `parse_failure_rate`、`field_missing_rate`、
`llm_schema_failure_rate`、`evidence_rejected` 与漏斗各级计数 → 达标后才开 `--live`。
