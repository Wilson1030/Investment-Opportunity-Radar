...
#### 5.2.3 ★★ 15 条真实公告抽样：一致率 27%，但问题在规格不在模型
...
第 04 条与第 03 条是**同类内容**（都是「重大资产重组部分限售股份上市流通」），
却分别被判成 `RESTRUCTURING` 与 `OTHER` —— 模型对同类内容的判定**不稳定**。
prompt 里的「正文优先」规则只能压住一部分，剩下的需要：
- 人工抽样继续跟踪（这正是 §5.3 要求的）
- 或考虑把「标题关键词与正文判定冲突」的样本单独送审

##### 结论：一致率不是正确的指标

真正该看的是**下游影响**：一条公告被判错，是否会让策略漏掉机会或误判机会？
按这个标准：
- 缺口 1 是**真问题**（会让 C1 漏判 → 核心策略失效）→ 已修
- 缺口 2 是**真问题**（会丢 13% 的公告）→ 已修
- 「一致率低」本身**不是问题** —— 规则层只是廉价预筛，语义归 LLM 是设计意图

> **下一步**：用修好的版本重跑 15 条，得到修复后的基线，再由人工打勾确认准确率 ≥ 80%。
