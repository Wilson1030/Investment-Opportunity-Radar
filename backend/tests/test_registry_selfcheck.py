...
def test_exactly_one_strategy_is_implemented():
    assert implemented_types() == (ThesisType.RESTRUCTURING,)
    assert len(designed_types()) == 9
...
def test_implementation_status_matches_registry():
    ...
    for code, status in statuses.items():
        if code is not ThesisType.RESTRUCTURING:
            assert status is StrategyStatus.DESIGNED
